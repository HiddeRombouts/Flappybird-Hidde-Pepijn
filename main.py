...
import time
import neat
import pickle # Om de beste vogel mee te bewaren
pygame.font.init() 
...
class Bird:
    ...
    def move(self):
        self.tick_count += 1
        displacement = self.vel * (self.tick_count) + 0.5 * (3) * (
            self.tick_count)**2
        if displacement >= 16:
...
def eval_genomes(genomes, config):
    ...
    while run and len(birds) > 0:
        clock.tick(100) #framerate van 100
        ...
        for i, bird in enumerate(birds):
            ...
            if bird.y > FLOOR - 50 or bird.y < 0:
                ge[i].fitness -= 10 #straf voor uit het scherm vliegen
        ...
        birds_to_remove = sorted(set(birds_to_remove), reverse=True)
        for i in birds_to_remove:
            try:
                nets.pop(i)
                ge.pop(i)
                birds.pop(i)
        ...
        for i, bird in enumerate(birds):
            if bird.y + bird.img.get_height() - 10 >= FLOOR or bird.y < -50:
                ge[i].fitness -= 5
                birds_dead.append(i)
        for i in sorted(set(birds_dead), reverse=True):
            try:
                nets.pop(i)
                ge.pop(i)
                birds.pop(i)
...
#laden van config apart voor hergebruik
